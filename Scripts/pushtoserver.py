...
try:
    for event in i.event_gen():
        if event is not None:													#if a new event exists
            ...
            if "IN_CLOSE_WRITE" in type_names:									#if new closed file in subscribed folder
                print("File created...")
                subprocess.call(["scp", filepath + "/" + filename, serverpath])	#push file to Habitat Server
                print("File uploaded")
                os.remove(filepath+"/"+filename)								#remove file from pseudo-satellites
                print("File deleted")
finally:
